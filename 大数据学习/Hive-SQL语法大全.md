...
>内部表Hive管理，默认`/user/hive/warehouse`；
>
>外部表随意，LOCATION关键字指定；被认为并非Hive所拥有的表，只是临时关联数据去使用；
>外部表和数据是相互独立的，可以先有表再有数据也可以先有数据再有表；
>
...
### 删除表
>
>删除内部表，表信息以及表数据会被全部删除
>
>清空表：`TRUNCATE TABLE '表名称'`（清空表内容保留表，仅限于Hive所管理的内部表）
>
```sql
DROP TABLE tbl;
```
...
### 分区操作
>分区表其实就是分了目录存放数据；PARTITIONED BY字段不能是表中存在的字段；
>
>分桶表是将一个文件拆分成多个文件存放（拆分到固定数量的不同文件中进行存储）；CLUSTERED BY的字段必须是表中的字段；
**创建分区表**
...
>
> 落桶的依据是CLUSTERED BY字段；分桶列的值进行hash取模运算；需要出发MapReduce，所以需要INSERT+SELECT而非LOAD；
>
> 基于分桶列（CID），单值过滤，双表JOIN，自动归并组（GROUP BY），减少扫描，提升性能；
...
==分桶表无法使用LOAD DATA进行数据加载==只能使用INSERT+SELECT==

所以需要一个表进行中转（临时表），中转表LOAD DATA，然后再INSERT+SELECT+CLUSTER BY；
...
将数据文件加载到表
>如果数据在HDFS，那么原文件会消失（本质是mv移动）
>
>如果数据在本地，需要带上`LOCAL`，否则不需要；
>
>`LOAD`方式不走MapReduce程序，小文件加载速度快；
```sql
...
- `bin/hive -e 'sql' > export_file` 将`sql`（SQL语句）结果重定向到导出文件中
...
#### **map类型**
- 映射类型：map类型其实就是简单的指代：key-value型数据格式。
- 建表语句（示例）：
```sql
...
#### **struct类型**
- 结构类型：在一个列中存入多个子列，每个子列允许设置类型和名称；
- 建表语句（示例）：
```sql
...
按顺序抽取，每次条件不变，结果不变；
```sql
SELECT ... FROM tb1 TABLESAMPLE(num1 ROWS | num2 PERCENT | num3(K|M|G));
```
抽取num1条数据，抽取num2百分比的数据，抽取num3大小的数据（后接单位）
...
**Hive中的函数主要分两大类：内置函数（Built-in Functions）和用户定义函数UDF（User-Defined Functions）**
